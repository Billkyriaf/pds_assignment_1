...
/**
 * Runs the OpenMP implementation of the algorithm. Function takes two arguments the path to the .mtx file and the number of threads. 
 * 
...
int main(int argc, char **argv) {
    ...
    gettimeofday(&begin, 0);
    openmpProduct(&input, &output, nthreads);
    gettimeofday(&end, 0);

    printf("NThreads: %d. Time for triangle calculation (OpenMP): %.5f seconds.\n", nthreads, measureTime(begin, end));
    printf("Number of triangles (OpenMP algorithm) is: %d\n", output.triangles);
